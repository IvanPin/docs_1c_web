  ...
  "products": [
    {
      "product": {
        ...
        "properties": {
          ...
          "ART": "123123",
          "url_detail_img": [
            "\\\\srv-1cbuh-nn.kokonn.local\\pic_unf\\20221029\\AR3344BKZ,WFM\\/B.jpg#2026-04-18T09:30:15Z"
          ]
        },
        "quantity": 100,
        "store_quantities": [
...
- `product` (object): основная информация о конкретном продукте.
  ...
  - `changed` (boolean, optional): признак того, что с момента прошлой выгрузки у товара изменились какие-либо параметры (реквизиты, свойства, фотографии, цены и т.п.). Если `false` или поле не передано — сайт может пропустить тяжёлые операции по этому товару (повторная загрузка изображений, переиндексация и т.д.).
  - `properties` (object, optional): произвольные атрибуты продукта в формате ключ/значение. Поддерживаются, в частности:
    - `url_detail_img` — массив путей или URL к фотографиям товара. Каждый элемент может содержать дату модификации файла через разделитель `#` в формате ISO 8601 (UTC): `<url>#<modified_at>`. Пример: `"\\\\srv-1cbuh-nn.kokonn.local\\pic_unf\\20221029\\AR3344BKZ,WFM/B.jpg#2026-04-18T09:30:15Z"`. Если дата не передана — сайт работает в режиме обратной совместимости и считает, что файл мог измениться.
    - `attr_sets` — `Артикул комплекта` (значение/значение в строку).
...
- `offers` (array, optional): список предложений (SKU) для продукта.
  - Каждый элемент — объект с полями предложения:
    ...
    - `changed` (boolean, optional): признак того, что с момента прошлой выгрузки у предложения изменились какие-либо параметры. Логика применения та же, что и у `product.changed`.
    - `properties` (object, optional): свойства предложения. Аналогично продукту, может содержать ключ `url_detail_img` со списком путей/URL к фотографиям. Для элементов массива `url_detail_img` действует тот же формат с датой модификации через `#`, что и у продукта. Старайтесь не дублировать характеристики, которые уже присутствуют у продукта; в предложении имеет смысл передавать только уникальные атрибуты, специфичные для данного SKU (например, размер кольца).
...
- Связка `changed` + дата модификации в `url_detail_img` позволяет сайту обновлять фотографии сразу при получении выгрузки, а не ждать ночного регламентного задания. Рекомендуемая логика со стороны 1С:
  - выставлять `changed=true` у товара/предложения, если с момента прошлой выгрузки изменился любой реквизит, свойство, цена, остаток или состав фотографий;
  - для каждой ссылки в `url_detail_img` передавать дату модификации файла (берётся из хранилища/файловой системы, не из 1С).
  - Рекомендуемая логика со стороны сайта: при `changed=false` и совпадении дат модификации файлов — пропускать повторную загрузку изображений; во всех остальных случаях — перекачивать изменившиеся файлы.
