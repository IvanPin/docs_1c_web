...
- `id` (string): уникальный идентификатор операции в 1С (GUID).
- `type` (string): тип операции — `"accrual"` (начисление) | `"writeoff"` (списание).
- `amount` (number): сумма в бонусных единицах (всегда положительное число).
- `description` (string, optional): человекочитаемый комментарий.
- `orderId` (string, optional): номер заказа, к которому привязана операция.
- `date` (string): дата и время операции в формате ISO 8601 с timezone.
- `expiresAt` (string|null): дата сгорания бонусов (ISO 8601) или `null`, если бессрочные (актуально только для `type: "accrual"`).
...
- Порядок записей в `bonuses[]` — от новых к старым (`date` desc).
