...
- Исключает позицию с `canceled: true` из состава заказа (обнуляет её в отгрузке).
- Сохраняет в истории заказа факт того, что позиция была удалена из отгрузки — сама история по заказу остаётся доступной.
- Применяет переданную сумму заказа.
...
> **Открытый вопрос: возврат денег при частичном выкупе оплаченного заказа.**
> Сейчас сценарий покрывает только оплату при получении (`Оплата при получении`, код `11`), где расхождение по сумме не требует возврата: клиент платит за то, что фактически забрал.
> Для заказов, оплаченных онлайн (`Оплата онлайн`, код `27`), механика не определена: в 1С возврат части суммы отдельно не фиксируется — деньги просто учитываются на балансе организации, признака частичного возврата в обмене нет. Возможный вариант со стороны сайта — уведомлять менеджера о необходимости вернуть разницу вручную, но операция требует отдельного согласования.
> Вопрос вынесен в отдельное обсуждение и на текущем этапе не реализуется.
